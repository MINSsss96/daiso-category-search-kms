...
OUTPUT_FILE = os.path.join(BASE_DIR, "logic", "category_breakdown_report.txt")
...
def main():
    ...
    # Structure: categorized_data[category] = {'MATCH': [], 'MISMATCH': []}
    ...
    for cat in categories:
        ...
        lines.append(f"## {cat} (Total: {total}, Accuracy: {accuracy:.1f}%)")
        
        lines.append(f"  [MISMATCHING QUESTIONS] ({len(mismatch_list)})")
        if mismatch_list:
            for item in mismatch_list:
                lines.append(f"    - Q: {item['q']}")
                lines.append(f"      Intention: {item['i']}")
                lines.append(f"      Extracted: {item['k']}")
        else:
            lines.append("    (None)")
        ...
        if match_list:
            # Limit printed matches to save space? User asked for "each type... matching question... mismatching question"
            # I will list all of them but maybe compact format.
