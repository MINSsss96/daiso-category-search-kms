...
JSON_PATH = os.path.join(BASE_DIR, "database", "qusetion_intention_1question.json")
...
def load_questions():
    ...
    with open(JSON_PATH, "r", encoding="utf-8") as f:
        ...
        # Handle list format (direct list of question objects)
        if isinstance(data, list):
            return [item.get("question") for item in data if isinstance(item, dict) and "question" in item]
            
        # Handle dict format (wrapper object with "questions" key)
...
def main():
    script_start_time = time.time() # Start total timer
    ...
    output_txt_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "extracted_keywords_1question.txt")
    ...
    with open(output_txt_path, "w", encoding="utf-8") as f:
        for i, q in enumerate(questions):
            ...
            if "error" in result:
                ...
                # Format: Question -> Keyword (Gemini Latency: Xs, Tokens: [P:X, C:X, T:X])
                line = f"Q: {q} -> Keyword: {keyword}"
                meta_info = f"(Gemini Latency: {latency:.3f}s, Tokens: [P:{tokens['prompt']}, C:{tokens['completion']}, T:{tokens['total']}])"
    ...
    if latencies:
        ...
        stats_msg = "\n[Latency Statistics] No successful requests."

    completion_msg = f"\nAnalysis complete. Results saved to {output_txt_path}"
    time_msg = f"Total Script Execution Time: {total_duration:.2f} seconds"
    
    print(stats_msg)
    print(completion_msg)
    print(time_msg)
    
    # Append total time and stats to file
