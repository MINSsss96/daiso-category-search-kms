...
# Configure for Llama 3.2 (assuming Ollama local defaults)
client = OpenAI(
...
KEYWORDS_FILE = os.path.join(BASE_DIR, "logic", "extracted_keywords_part_refill_questions_100_llama.txt")
OUTPUT_FILE = os.path.join(BASE_DIR, "logic", "comparison_report_part_refill_questions_100_llama.txt")
...
def main():
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        ...
        for item in intentions:
            # Handle different key names in datasets
            q = item.get("question") or item.get("query")
            target = item.get("intention") or item.get("intent")
